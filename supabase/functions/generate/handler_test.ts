...
Deno.test("overview returns the model intro with sources", async () => {
  const res = await handle(post({ mode: "overview", artwork_id: "a" }), deps());
...
Deno.test(
  "persona falls back to a human-facing stub when the model fails",
  async () => {
    const res = await handle(
      post({
        mode: "persona",
        lang: "fr",
        onboarding: { motivation: "understand" },
      }),
      deps({ complete: failing }),
    );
    const body = await res.json();
    assert(
      body.persona_summary.includes("Ravi de vous accompagner"),
      body.persona_summary,
    );
    assert(
      !body.persona_summary.includes("motivation="),
      "fallback must not leak raw signals",
    );
  },
);
...
Deno.test(
  "hotspot falls back to the seed text when the model fails",
  async () => {
    const res = await handle(
      post({ mode: "hotspot", artwork_id: "a", hotspot_ids: ["h1"] }),
      deps({ complete: failing }),
...
Deno.test(
  "hotspot failure fallback stays in the requested non-French language",
  async () => {
    const res = await handle(
      post({
        mode: "hotspot",
    ...
    const { items } = await res.json();
    assertEquals(items[0].status, "ready");
    assert(items[0].text.includes("Kijk"), items[0].text);
    assert(!items[0].text.includes("seed"), "Dutch fallback reused the seed");
  },
);
...
Deno.test(
  "hotspot with empty hotspot_ids returns an empty items array",
  async () => {
    const res = await handle(
...
Deno.test(
  "followups falls back to stubs when model returns empty output",
  async () => {
...
Deno.test(
  "ask streams delta events then a done with the full text",
  async () => {
    const res = await handle(
...
Deno.test(
  "ask emits an error event when the stream fails before any token",
  async () => {
    const res = await handle(
      post({ mode: "ask", artwork_id: "a", question: "why?" }),
      deps({
...
Deno.test(
  "ask emits done (not error) when stream fails after some tokens",
  async () => {
    ...
    assert(
      ...
      "should not emit error after partial text",
    );
  },
);
...
Deno.test(
  "grounding facts reach the LLM messages (not just the sources count)",
  async () => {
    ...
    await handle(
      ...
      deps({
        readNotices: () =>
          Promise.resolve([
            { id: "n1", lang: "en", source: "rijks", text: "MARKER_FACT_42" },
          ]),
        complete: (messages) => {
...
Deno.test(
  "EN-pivot: handler grounds and cites only the EN notices",
  async () => {
    let captured: Array<{ role: string; content: string }> = [];
    const res = await handle(
      post({ mode: "overview", artwork_id: "a" }),
        ...
        readNotices: () =>
          Promise.resolve([
            ...
            { id: "en-w", lang: "en", source: "wikipedia", text: "EN_WIKI" },
            {
              ...
              lang: "nl",
              source: "wikipedia",
              text: "NL_WIKI_SHOULD_BE_DROPPED",
            },
          ]),
        complete: (messages) => {
          captured = messages as typeof captured;
    ...
    assertEquals(
      body.sources.map((s: { notice_id: string }) => s.notice_id).sort(),
      ["en-r", "en-w"],
    );
    ...
    assert(
      !prompt.includes("NL_WIKI_SHOULD_BE_DROPPED"),
      "NL notice leaked into grounding",
    );
  },
);

Deno.test(
  "ask threads history_summary as an earlier-context system message",
  async () => {
