...
Deno.test(
  "stubHotspotText only uses narration_text for French fallback",
  () => {
    // The DB seed is French in prod; only French fallback may speak it directly.
    assertEquals(
      stubHotspotText(
        {
          id: "h",
    ...
    const nl = stubHotspotText(
      { id: "h", title: "T", aspect: "A", narration_text: "real" },
      "nl",
    );
    assert(nl.includes("Kijk"), nl);
    assert(!nl.includes("real"), "Dutch fallback must not reuse the French seed");
  },
);
...
Deno.test(
  "buildGrounding labels notices by source/lang and passes short text in full",
  () => {
...
Deno.test(
  "buildGrounding trims an over-budget notice at a section boundary",
  () => {
    const huge =
      "lead.\n" +
      Array.from(
    ...
    const g = buildGrounding([
      { id: "n", lang: "en", source: "wikipedia", text: huge },
    ]);
...
Deno.test(
  "overviewPrompt embeds the grounding and the intro instruction",
  () => {
...
Deno.test(
  "stubPersona falls back to a human-facing note",
  () => {
    const out = stubPersona(
      { motivation: "stories", knowledge: "newcomer", depth: "quick" },
      "fr",
    );
    assert(out.includes("Ravi de vous accompagner"));
...
Deno.test(
  "stripNoiseSections drops boilerplate sections but keeps the lead and content",
  () => {
    const text =
      "Lead paragraph.\n== History ==\nReal content.\n== References ==\n[1] junk\n== External links ==\nhttp://x";
    ...
    assert(
      !out.toLowerCase().includes("external links"),
      "external links not stripped",
    );
  },
);

Deno.test("stripNoiseSections strips Dutch boilerplate headings too", () => {
  const out = stripNoiseSections(
    "Lead.\n== Zie ook ==\nx\n== Externe links ==\ny",
  );
...
Deno.test(
  "fitToBudget keeps whole sections only (never mid-section) and marks the trim",
  () => {
    const text =
      "lead\n" +
      Array.from(
...
Deno.test(
  "selectPivotNotices prefers EN when an EN wikipedia notice exists",
  () => {
    const notices = [
    ...
    assertEquals(
      out.map((n) => n.id),
      ["1", "3"],
    ); // only EN, both sources
  },
);

Deno.test(
  "selectPivotNotices falls back to NL when no EN wikipedia exists",
  () => {
    const notices = [
      { id: "1", lang: "en", source: "rijks", text: "a" },
    ...
    assertEquals(
      out.map((n) => n.id),
      ["2"],
    ); // pivot to the language that has the article
  },
);
...
Deno.test(
  "systemPrompt instructs the model to answer in the requested language",
  () => {
...
Deno.test("systemPrompt maps depth=quick to the short length hint", () => {
  ...
  assert(
    s.toLowerCase().includes("one or two sentences"),
    "expected short-length hint",
  );
});
...
Deno.test("systemPrompt maps depth=deep to the rich length hint", () => {
  ...
  assert(
    s.toLowerCase().includes("rich paragraph"),
    "expected long-length hint",
  );
});

Deno.test(
  "systemPrompt maps knowledge=newcomer to plain-words register",
  () => {
...
Deno.test(
  "systemPrompt maps knowledge=expert to precise vocabulary register",
  () => {
...
Deno.test(
  "systemPrompt maps motivation=stories to the story/people angle",
  () => {
...
Deno.test("systemPrompt defaults are applied when no profile is given", () => {
  ...
  assert(
    s.toLowerCase().includes("three or four sentences"),
    "expected standard depth default",
  );
  assert(
    s.toLowerCase().includes("understand"),
    "expected understand motivation default",
  );
});
...
Deno.test(
  "systemPrompt injects the visitor's own words, not the persona summary",
  () => {
    const s = systemPrompt(
      "fr",
      {
        free_text: "an architect who loves light",
        persona_summary: "curious beginner",
      },
      undefined,
    );
    assert(
      s.includes("an architect who loves light"),
      "expected free_text injected",
    );
    assert(
      !s.includes("curious beginner"),
      "persona_summary must NOT leak into the system prompt",
    );
  },
);
...
Deno.test("askPrompt adds point context when a coordinate is provided", () => {
  const p = askPrompt("What is that?", "FACTS: y", {
    point: { x: 0.5, y: 0.3 },
  });
...
Deno.test(
  "followupsPrompt embeds the grounding and the target language",
  () => {
