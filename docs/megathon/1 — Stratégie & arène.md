...
- **Décision révisée :** **Siffrein** = serveur/runtime cloud (Edge Functions, secrets LLM/STT/TTS, déploiement). **Adam/Codex** = app mobile (UI, AR, vue détail, hotspots, audio, chat). **Adam CEO** = contenu phares, business, pitch, logistique. **Designer** = UI/brand. (cf. [[2 — Tech & build]], M36)
- **Pourquoi :** la coupe est orthogonale : tout ce qui tient un secret ou tourne côté cloud reste chez Siffrein ; tout ce que le visiteur voit et manipule reste dans l'app ; les seules surfaces partagées sont le contrat `/generate`, `/transcribe`, TTS et les types.
...
| M31 | **Identification = chaîne à 2 étages.** Primaire = **ViroReact image tracking** (reco + ancrage **couplés** : il n'ancre que les cibles du set de référence). Repli = **capture du flux → modèle de vision (Claude vision) qui identifie l'œuvre** ; le positionnement bascule alors sur **overlay 2D** (pas d'ancre monde sans cible reconnue). Puis manuel / QR. → ramène la vision comme **filet concret**, plus seulement story de pitch. NB technique : ViroReact ne sait pas « détecter une position sans identité » ; un détecteur de rectangle générique serait un chantier séparé, hors démo. | ✅ acté |
| M32 | **Modèle runtime = open hébergé Nebius** (crédits builder kit 100$) → **bascule Claude API payant** si la qualité manque. Correction : **abonnement Claude.ai ≠ clé API** — pas de Claude « gratuit via abonnement ». Le port LLM reste swappable (ADR 0012), le contrat `/generate` ne change pas. | ✅ acté (à valider sur Nebius demain) |
| M33 | **STT = Voxtral**, cloud, derrière une **2e edge function `/transcribe`** (audio → texte ; requête courte, pas de socket long). STT on-device **écarté** (plus faible en milieu bruyant ; et Claude ne prend pas l'audio en entrée). Le STT ne fait que **transcrire**, pas raisonner. | ✅ acté |
| M34 | **Hotspots ancrés sans appel LLM au tap.** Première coupe : texte préchargé fixe et `f()` réservé au Q&A. Cette décision garde le principe clé (tap instantané) mais le mode de génération est **révisé par M35**. | ☑️ remplacé par M35 |
| M35 | **Hotspots personnalisés par profil à l'ouverture de l'œuvre.** Révision de M34 : le tap hotspot ne déclenche toujours pas le LLM, mais l'app lance en async, dès l'entrée en vue détail, **un `/generate mode=hotspot` par hotspot**, en parallèle. Le texte dit au visiteur est donc unique au profil/langue/session ; `narration_text` stocké devient seed/fallback si la génération tarde. | ✅ acté |
| M36 | **Répartition build révisée.** Siffrein possède le **serveur/runtime cloud** : Edge Functions `/generate` et `/transcribe`, clés LLM/STT/TTS, secrets, déploiement. Adam/Codex possède l'**app** : UI, design, AR point bleu ancré, vue détail, hotspots, lecteur audio, chat, états de chargement/fallback. | ✅ acté |
