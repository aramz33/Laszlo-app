...
| M34 | **Hotspots ancrés = contenu préchargé** (texte), **pas d'appel `f()` live au tap**. La génération LLM live est réservée au **Q&A** : chat libre, **point placé par l'utilisateur** (endroit arbitraire de l'œuvre), **conversation depuis un hotspot**. Motif : le tap hotspot = cœur du wow → chemin chaud sans latence ni risque LLM ; le live ne sert qu'à ce qui **ne peut pas** être préchargé. Révise le `mode=hotspot` d'ADR 0014. 🟡 ouvert : texte hotspot fixe vs pré-généré par profil. | ✅ acté |
